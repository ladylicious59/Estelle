...
export const handleStripeWebhook = async (req, res) => {
    ...
    switch (event.type) {
      ...
      `).run(`sub_${Date.now()}`, userId, stripeCustomerId, stripeSubscriptionId, plan, 'active', new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() // Placeholder, should get from subscription object
            );
            break;
        }
